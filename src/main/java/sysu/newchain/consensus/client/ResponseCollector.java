 ...
 * @param <V> 投票值
 */
public class ResponseCollector<T, V> {
	static final Logger logger = LoggerFactory.getLogger(ResponseCollector.class);
	...
	public boolean create(String index, CompletableFuture future) {
		Entry<T, V> entry = new Entry<T, V>(future);
		return requests.putIfAbsent(index, entry) == null;
	}
